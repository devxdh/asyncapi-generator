...
| `conditionalGeneration` | Object[String, `{ subject?: String, parameter?: String, validation: Object }`] | An object containing all the file paths or directory names that should be conditionally rendered. Each key represents a file path or directory name and each value must be an object containing `validation` (a JSON Schema object) and exactly one non-empty selector: `subject` (a JMESPath query) or `parameter` (a template parameter name). Both selectors cannot be defined at the same time, and one of them is required. The path should be relative to the `template` directory inside the template. |
